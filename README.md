...
In low-disk mode, `QUANTA_LOW_DISK_FLOAT32` defaults to `false` (keep `float64` unless explicitly overridden).
...
- Step-1 target now includes `FILTER` (or `factors-filter`) to run the filter pipeline only and exit (no backtest).
  It prints `experiment`, `quality_range`, `final_selected`, and writes output under
  `data/factorlib/selected/<experiment>_factors_filter_q<quality>_n<count>_<timestamp>.json`
  (plus stable alias `<experiment>_factors_filter_latest.json`).
- After picking a single custom library, the wizard now offers a one-click "production dedup preset" that auto-fills:
...
```bash
# Run backtest with explicit thread cap
...
# Filter pipeline only (no backtest): print final kept factor count
./scripts/run_backtest_safe.sh \
  --factors-filter \
  --library data/factorlib/all_factors_library_paper_reproduction_ds.json
```
...
- Correlation dedup now defaults to `two_stage`: Stage-1 exposure-corr coarse filter + Stage-2 IC-series corr fine filter.
- Stage-1 no longer truncates by global TopN; `--dedup-topn` is applied only after Stage-2 to preserve diversity breadth.
- Cluster linkage now supports `complete|connected` (default `complete`) to reduce chain-merge over-pruning risk.
